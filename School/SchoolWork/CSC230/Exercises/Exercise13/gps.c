...
void readCity( FILE *fp, struct City *cty ) {
  ...
  fscanf( fp, "%s %lf %lf", cty->name, &cty->latitude, &cty->longtitude );
}
...
int main( int argc, char *argv[] )
{
  ...
  struct City *cty = ( struct City *)malloc ( n * sizeof( struct City ) );
  ...
  while ( scanf( "%lf%lf", &lat, &lon ) == 2 ) {
    ...
    char *c;
    for ( int i = 0; i < n; i++ ) {
      dis = globalDistance( lat, lon, cty[i].latitude, cty[i].longtitude );
      if ( dis < min ) {
        min = dis;
        c = cty[i].name;
      }
